...
import { request, checkVal, dt_formating_sec, recalcRowsRet } from "../views/globals";
...
export default class BarcodesSView extends JetView {
    config() {
        ...
        var delB = (pars) => {
            ...
            let res = request(url, params, !0, h.$scope.app).response;
            res = checkVal(res, 's');
            if (res) {
                th.remove(item.id);
        ...
        var editBarCode = (id_spr, parse) => {
            ...
            if (res) {
                ...
                th.remove(rem);
                url = "getBar";
                let r1 = checkVal(request(url, params, !0, th.$scope.app).response, 's');
