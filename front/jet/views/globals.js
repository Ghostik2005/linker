...
export function parseToLink(item) {
    ...
    checkVal(request(url, params, !0, app).response, 's');
...
export function get_bars(th, id_spr) {
    ...
    let res = request(url, params, !0, th.app).response;
    res = checkVal(res, 's');
    if (res) {
        $$("bars_dc").clearAll();
        $$("bars_dc").parse(res);
...
export function refReload(url, params, async, app) {
    if (async) {
        ...
        let data = request(url, params, !0, app).response;
...
export function singleRefReload(app, method, parent) {
    let url = method;
    let params = { "user": app.config.user };
    return refReload(url, params, parent, app)
}
...
export function gen_params(inp_params) {
    ...
    let user = app.config.user;
    let url = inp_params.method;
    let params = {
        "user": user, "search": search_str, "start": inp_params.start, "count": inp_params.count,
            ...
            node.onclick = function (e) {
                ...
                if (e.target.className === "header-checkbox" || e.target.className === "threeSt-indeterminate") {
                    ...
                    if (config.indeter || config.checked) {
                        ...
                        let url = (config.inputConfig.getIdMethod) ? config.inputConfig.getIdMethod : undefined;
                        if (url) {
                            node.disabled = 1;
                            request(url, params, 0, master.$scope.app).then(function (data) {
                                node.disabled = 0;
                                data = checkVal(data, 'a');
                                if (data) {
...
export function getRefs(app, sync) {
    ...
    let params = { "user": app.config.user };
    if (sync) {
        let data = checkVal(request(url, params, !0, app).response, 's');
        setRefs(data);
    } else {
        request(url, params, 0, app).then(function (data) {
            data = checkVal(data, 'a');
...
export function get_prcs(th, id_vnd) {
    ...
    request(url, params, 0, th.app).then(function (data) {
        data = checkVal(data, 'a');
        if (data) {
            $$("prcs_dc").clearAll();
            if (data.length > 0) {
...
export function request(url, params, mode, app) {
    let r_url = (app) ? app.config.r_url + "?" + url : url
    var req = (mode === !0) ? webix.ajax().sync().headers({ 'Content-type': 'application/json' }).post(r_url, params, { error: after_call })
        : webix.ajax().timeout(90000).headers({ 'Content-type': 'application/json' }).post(r_url, params, { error: after_call })
    return req
}
