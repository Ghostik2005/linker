...
"""
09.10.2018
make possible to parse thi BytesIO object as a dbf file
"""
...
class DBF(object):
    ...
    def __init__(self, filename, encoding=None, ignorecase=True,
        ...
        self.f_obj = False if isinstance(filename, str) else True 
        ...
        if self.f_obj:
            self._r_f_obj(self.filename)
        else:
            with open(self.filename, mode='rb') as infile:
                self._r_f_obj(infile)
    ...
    def _count_records(self, record_type=b' '):
        ...
        if self.f_obj:
            count = self._c_rec(self.filename)
        else:
            with open(self.filename, 'rb') as infile:
                count = self._c_rec(infile)
        return count

    def _c_rec(self, infile):
        count = 0
    ...
    def __repr__(self):
        if self.loaded:
            ...
            status = 'unloaded'
        return '<{} DBF table {!r}>'.format(status, str(self.filename))
