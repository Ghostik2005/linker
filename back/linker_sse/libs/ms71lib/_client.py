...
__version__ = "%s.4.15"  % sys.version.split('.', 1)[0]
...
class Transport:
    ...
    def single_request(self, host, handler, request_body, verbose=False):
        ...
        try:
            ...
            if location:
                ...
                host2 = urllib.parse.urlparse(location).netloc
    ...
    def send_request(self, host, handler, request_body, debug):
        ...
        if self.accept_gzip_encoding and gzip:
            connection.putrequest("POST" if request_body else "GET", handler, skip_host=self._host_name, skip_accept_encoding=True)
            headers.append(("Accept-Encoding", "gzip"))
        else:
            connection.putrequest("POST" if request_body else "GET", handler, skip_host=self._host_name)
