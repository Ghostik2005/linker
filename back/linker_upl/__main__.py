...
__version__ = "20.041.1140"  # оптимизирован sql запрос на сведенени по кодам
# __version__ = '20.041.1025' # оптимизированны sql запросы на удаление и вставку
# __version__ = '20.023.1655' # не отправляем в пропущенные позиции от Регион-В
# __version__ = '20.022.1455' # отделили Регион-В от Хелса
# __version__ = '20.021.1253' # не отправляем в пропущенные позиции от Стандарта
# __version__ = '20.021.1220' # изменено формирование имени файлов на закачку
# __version__ = '20.020.1620' # добавленна обработка на антей
...
def main():
    ...
    sys.APPCONF["api"] = libs.API(
        Lock=sys.APPCONF["Lock"],
        log=sys.APPCONF["log"],
        w_path=w_path,
        p_path=p_path,
    )

    threads, processes = prepare_server(
        Lock=sys.APPCONF["Lock"], api=sys.APPCONF["api"]
    )
    rc = 0
    try:
    ...
    if fname:
        if "POST" == _rm:
            ...
            if data and fname:
                ...
                content = sys.APPCONF["api"].upload_file(
                    fname, data, source, callback, x_hash=env["X-API-KEY"]
                )
    else:
        if "POST" == _rm:
