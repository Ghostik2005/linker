...
class DBsync:
    def Load(self):
        ...
        tmpArray = []
        error = ''
        tmpArray, error = xlsx2tsv.load('b2.xlsx')
        code = '1251'
        i = 0
        try:
            print len(tmpArray)
            for rec in tmpArray:
                ...
                if i != 0:
                    ...
                    sql = """INSERT INTO SPRIMP_B2 (CODE1, GRUP ,VISIBLEGR ,CODE) VALUES ('%s', '%s', %s, '%s')""" %\
                    (splitResult[0].encode(code),splitResult[1].encode(code), splitResult[2], splitResult[3].encode(code))
